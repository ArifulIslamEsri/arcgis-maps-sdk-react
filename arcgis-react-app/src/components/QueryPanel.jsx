...
// ─────────────────────────────────────────────────────────────
// Demonstrates: Query object, where clause (SQL filter),
// queryFeatures(), FeatureSet, goTo() to zoom to results
//
// Users type a population threshold and the app queries the
// US States feature service, listing and highlighting matches.
// ─────────────────────────────────────────────────────────────
...
export default function QueryPanel({ view }) {
  ...
  const runQuery = async () => {
    ...
    try {
      // ── Build where clause ──────────────────────────────────
      // SQL-style string sent to the feature service.
      ...
      // ── Query object ────────────────────────────────────────
      const layer = new FeatureLayer({ url: SERVICE_URL });
      const query = {
        where: whereClause,
        outFields: ["STATE_NAME", "POPULATION", "SUB_REGION"],
        returnGeometry: true, // need geometry to highlight + zoom
      };

      // ── queryFeatures() returns a FeatureSet ────────────────
  ...
  return (
    <div style={styles.panel}>
      ...
      <p style={styles.desc}>
        Sends a SQL <code>where</code> clause to the feature service via{" "}
        <code>queryFeatures()</code>. Matching states are highlighted in yellow
        and the map zooms to them.
      </p>
